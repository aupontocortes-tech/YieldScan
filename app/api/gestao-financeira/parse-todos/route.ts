...
import { buildGfTodosSystemPrompt, GF_OPENAI_MODEL, todosFromApi } from '@/lib/gestao-financeira/todos-llm-shared'
  ...
  try {
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: buildGfTodosSystemPrompt() },
    ...
    if (!items.length) {
      return NextResponse.json({
        result: null,
        error: 'Não consegui extrair afazeres. Ex.: «Amanhã dentista às 14h» ou «Sexta pagar luz».',
        usage: { model, promptTokens, completionTokens, estimatedUsd },
      })
    }

    const result: GfTodoParseResult = { items, source: 'openai' }
